...
- 准备度通过 -> 先产出 `module_plan.md`，人工评审批准后才进编码。
- 发现设计缺口 -> 生成阻塞反馈，建议回到最近的设计阶段。
- 收到实现 bug / lint 风格问题 -> 回到步骤 1，按当前输入修 plan、RTL 与交接物。
...
- 未产出 `module_plan.md`，或 plan 未获人工批准就开始写 RTL。
- 输出端口使用 `wire` 直接组合输出，没有对应的 `reg` 信号和时序更新块。
- 状态机的状态转移（next-state）组合逻辑没有以当前状态寄存器为选择条件使用顶层 `case` 语句组织。

### 证据与交付不一致

- 交付前无法把 `module_plan`、RTL、`verification_input` 与输入来源逐项对上。
- RTL 无法逐条回对 `module_plan.md` 的控制路径与数据路径设计。
- review 结论只能口头解释，不能定位到真实 RTL 文件、信号或逻辑块。
- plan、verification_input 或最终回复引用了当前 RTL 不存在的信号、逻辑块或路径。
...
- 从用户输入或用户指定文档中提取模块设计输入；若已有成文的模块设计文档，优先读取该文件。
...
- 编码假设只能来自用户输入、指定文档、项目已有模块设计文档和输入准备度检查结论；不要把上一阶段 stdout、路由摘要或未落盘对话上下文当作稳定输入。
...
简单模块也必须先通过输入准备度检查。输入可以来自用户当前输入、用户指定文档或项目已有模块设计文档。
...
- 照已批准的 `module_plan.md` 分段落地 RTL，不要一口气拼完整文件；每段对应 plan 的一部分：
  1. `module`块 与 参数和端口列表：先落module块声明、参数、端口方向、位宽、端口注释和基础信号声明。
  2. 控制路径：按 plan 的控制路径设计落 FSM、握手、仲裁、使能、状态转移、异常/边界控制和寄存器更新，并为每个主要功能块写明职责。
  3. 数据路径：按 plan 的数据路径设计落数据选择、运算、buffer、pipeline 数据寄存器和输出数据组合逻辑，并为关键功能块写明输入依赖和输出结果。
  4. 整体收敛：最后补齐输出端口寄存器时序更新块、CBB/IP 例化、输出绑定、默认值、注释和文件级一致性；每个 output 端口必须有对应的 `reg` 信号和时序更新块。
- 分段推进节奏：若环境提供 `TodoWrite` 或等价 todo 工具，进入编码时先把上述四段建立为编码 todo，逐项 `in_progress` -> 落盘该段 -> `completed`，一次只聚焦一段；这是执行节奏引导，帮助真正分段落地，不替代 plan 与 review 门禁。
- 每一段完成后，检查该段是否仍服从 `module_plan.md`、模块设计输入和输入准备度检查结论；不得把关键结构、延迟、吞吐、顺序、并发或异常语义替换成“功能近似”方案。
- 若编码中发现 plan 需实质调整（改动控制/数据路径结构），回到步骤 3 更新 plan 并重新送评审，不在编码里偷偷改设计；仅措辞或命名对齐不必重新评审。
...
- plan 侧：`module_plan.md` 已获批准，且 RTL 能逐条回对 plan 的宏观结构、控制路径与数据路径设计；plan 与 RTL 若有偏离，已按步骤 3/4 消解并同步 plan。
...
- 一致性侧：module_plan、verification_input 与最终回复中的关键信号、逻辑块、阶段路径和文件路径，能在当前 RTL 与工作区逐一定位。
...
- 报告侧：review 通过后，写入 `<module_name>/review_report.md`，记录 `module_plan.md` 回对结论、`<module_name>_checklist` 与 `coding_standards.md` 的 review 结论；coding standards 结论必须包含端口注释、功能点注释、功能块注释和关键实现注释的检查结果。

- Review 不通过时，回到步骤 4 修改 RTL，并同步更新 `verification_input.md`；若涉及设计调整，回到步骤 3 更新 plan 并重新送评审。重复直到 `module_plan.md` 回对、`<module_name>_checklist` 与 `coding_standards.md` 全部通过后，才写入 `review_report.md` 并交付。
...
- 更新规则：RTL、输入假设、CBB/IP 路径或关键行为变化后，必须同步更新受影响的 `module_plan.md`、`verification_input.md` 和 `review_report.md` 内容。

## 完整交付与局部修复

- 完整模块交付：必须生成或更新 `module_plan.md`、RTL、`verification_input.md` 和 `review_report.md`。
- 局部 RTL 修复：只更新受影响的 RTL 和交接物章节；未受影响的输入假设、plan、验证关注点和 review 结论可保持不变，但最终回复必须说明影响范围。
- 若局部修复改变接口、复位、时序、异常、寄存器语义、CBB/IP 路径、控制/数据路径结构或验证关注点，按完整模块交付重新执行步骤 1-6（含 plan 更新与重新送评审）。

## 阻塞、反馈与重入

先区分“可整改问题”和“阻塞问题”：

- 可整改问题：RTL 实现、风格、lint、静态检查、仿真反馈、`module_plan.md` 回对、`<module_name>_checklist` 或 `coding_standards.md` review 不通过。此类问题留在本技能内整改：从步骤 1 重新确认输入，回到步骤 4 修改 RTL（涉及设计调整则回步骤 3 更新 plan 并重新送评审），重复直到 review 全部通过。
...
- `blocked_step`: 当前阻塞步骤，如 `input-readiness` / `plan-review` / `coding` / `verification-handoff` / `final-review`
...
- 输入准备度不通过时，不写 plan、不写 RTL；补齐输入后从步骤 1 重入。
- `module_plan.md` 未获人工评审批准时，不进入编码；用户要求修改时更新 plan 后重新送评审。
- 写 plan 时发现需要未闭合的 what 决策（接口、边界、对外行为、时序契约），不在 plan 里就地补决策；回退步骤 1 按逐项确认方式询问用户。
...
- 代码修复、lint 修复或仿真反馈修复后，从步骤 1 重新确认输入；按「完整交付与局部修复」更新受影响的 `module_plan.md`、RTL、`verification_input.md` 和 `review_report.md` 后再交付。
