...
如果用户要的是 RTL/数字模块**方案框图**，即使描述中包含 valid/ready、req/ack 或总线事务，也应使用 `tech-diagram.md`；只有输出目标是波形/时序/逐拍关系时才使用本文件。
...
| setup/hold | 采样前后稳定时间 | 双向箭头 + 独立文字 `t_su` / `t_h` |
...
WaveJSON 常见写法：用 `node` / `edge` 表示采样关系。注意：这些字段超出 `scripts/wavejson_to_drawio.py` 的自动转换范围；遇到这种输入时，先用人工绘制方式保留采样语义，或先生成基础波形后手工补充采样箭头和文字。
...
- 在采样时钟沿放小三角或短竖线。
- 从采样边沿画箭头指向被采样的数据区间。
- 箭头旁用独立 text vertex 标注 `sample`、`latch`、`capture` 或中文「采样」。
...
- 箭头旁用独立 text vertex 标注 `t_pd`、`t_co`、`latency`、`N cycles`。
...
7. **自检**：所有跳变点 X 对齐，文本不压线，X/Z 区分清楚，箭头不穿过主要文字；注释文字用透明 text vertex，必要时移到波形外侧留白。
8. **校验**：保存为 `.drawio` 后运行 `python <drawio-chip skill directory>/scripts/validate_drawio.py --no-edge-labels --transparent-text-labels <file.drawio>`；验证失败先修 XML。
  ...
  <diagram name="Timing Primitives" id="timing-primitives">
    <mxGraphModel dx="1000" dy="700" grid="0" gridSize="10" guides="1" tooltips="1" connect="1" arrows="1" fold="1" page="1" pageScale="1" pageWidth="520" pageHeight="380" math="0" shadow="0">
      <root>
        ...
        <mxCell id="62" value="" style="endArrow=block;startArrow=block;html=1;rounded=0;strokeColor=#6C8EBF;fontColor=#6C8EBF;" edge="1" parent="1"><mxGeometry relative="1" as="geometry"><mxPoint x="180" y="320" as="sourcePoint"/><mxPoint x="220" y="320" as="targetPoint"/></mxGeometry></mxCell>
        <mxCell id="63" value="" style="endArrow=block;startArrow=block;html=1;rounded=0;strokeColor=#82B366;fontColor=#82B366;" edge="1" parent="1"><mxGeometry relative="1" as="geometry"><mxPoint x="220" y="340" as="sourcePoint"/><mxPoint x="260" y="340" as="targetPoint"/></mxGeometry></mxCell>
        <mxCell id="65" value="t_su" style="text;html=1;align=center;fontSize=11;fontColor=#6C8EBF;fillColor=none;strokeColor=none;" vertex="1" parent="1"><mxGeometry x="180" y="307" width="40" height="16" as="geometry"/></mxCell>
        <mxCell id="66" value="t_h" style="text;html=1;align=center;fontSize=11;fontColor=#82B366;fillColor=none;strokeColor=none;" vertex="1" parent="1"><mxGeometry x="220" y="327" width="40" height="16" as="geometry"/></mxCell>
...
- 交付前必须通过 `python <drawio-chip skill directory>/scripts/validate_drawio.py --no-edge-labels --transparent-text-labels` 的结构、ID、edge 引用、线文字和透明注释检查。时序图/波形图不使用 `--no-text-line-overlap`，因为总线文字和采样标注允许落在波形通道附近。

## 常见问题

- **画成协议模板而不是通用时序图**：先拆原语，再组合；不要默认套协议模板，除非请求本身指定某个协议。
